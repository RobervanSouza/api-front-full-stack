...
const editarTurismo = async ({ _id, nome, genero, ano, imageUrl }) => {
    await fetch(`${url}/${_id}`, {
        method: 'put',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, genero, ano, imageUrl })
    });
...
const createTurismo = (rest) => {
    const { _id, nome, genero, ano, imageUrl } = rest;
    ...
    submit.addEventListener('submit', (event) => {
        ...
        editarTurismo({ _id: editId, nome, genero, imageUrl, ano });
        fecharModal();
    });
