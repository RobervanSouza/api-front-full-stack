const url = 'https://api-back-kappa.vercel.app/celular';
...
const editarTurismo = async ({ _id, nome, marca, valor, imageUrl }) => {
    await fetch(`${url}/${_id}`, {
        method: 'put',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, marca, valor, imageUrl })
    });
...
const createTurismo = (rest) => {
    const { _id, nome, marca, valor, imageUrl } = rest;
    ...
    const valor1 = restElementos('p', `Valor:  ${valor}`);
    ...
    submit.addEventListener('submit', (event) => {
        ...
        editarTurismo({ _id: editId, nome, marca, imageUrl, valor });
        fecharModal();
    });
