...
const editarTurismo = async ({ _id, nome, autor, descricao, imageUrl }) => {
    await fetch(`${url}/${_id}`, {
        method: 'put',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, autor, descricao, imageUrl })
    });
...
const createTurismo = (rest) => {
    const { _id, nome, autor, descricao, imageUrl } = rest;
    ...
    const autor1 = restElementos('p', `Autor:  ${autor}`);
    ...
    submit.addEventListener('submit', (event) => {
        ...
        editarTurismo({ _id: editId, nome, autor, imageUrl, descricao });
        fecharModal();
    });
